...
export default function DataTableReserves() {
    const [reserves, setReserves] = useState(null);

    const columns = [
        { field: 'id', header: 'ID' },
        { field: 'name', header: 'Name' },
        { field: 'date', header: 'Review' },
        { field: 'time', header: 'Rating' },
        { field: "numberGuests", header: "Guests" },
        { field: "reservationId", header: "Cancel Code" },
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const response = await fetch(API_URL + 'api/project/GetReserves');
    ...
    return (
        <div className="card p-fluid m-4 text-center">
            <DataTable value={reserves} editMode="cell" className='bg-slate-200 border-black border-2 text-center' tableStyle={{ minWidth: '50rem' }}>
                {columns.map(({ field, header }) => (
                    <Column
                        ...
                        className='bg-red-100 border-black border-2 width-fit'
                    />
                ))}
            </DataTable>
        </div>
