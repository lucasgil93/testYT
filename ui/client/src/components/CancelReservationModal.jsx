...
function CancelReservationModal({ onClose }) {
  ...
  const handleCancel = async () => {
    ...
    try {
      // Ask for confirmation before making the fetch request
      if (
        window.confirm(
          "Your reservation is about to be cancelled, are you sure?"
        )
      ) {
        const response = await fetch(
          API_URL + `api/project/DeleteReserve?reservationId=${cancelId}`,
          {
            method: "DELETE",
          }
        );
        ...
        if (response.ok) {
          ...
          throw new Error(
            errorData.error || "Failed to cancel the reservation"
          );
        }
      } else {
        showInfo("Reservation cancellation cancelled");
      }
    } catch (error) {
